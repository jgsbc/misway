...
Decision classes used: `ADOPT_DOCUMENT`, `ADOPT_DOCUMENT_WITH_MINIMAL_RECONCILIATION`, `HISTORICAL_EVIDENCE_ONLY`, `SUPERSEDED_BY_MAIN`, `DEFER_RUNTIME_TO_BASE_00`, `DEFER_RUNTIME_TO_SYS_00`, `DEFER_RUNTIME_TO_VS1_00`, `REJECT`.
...
- **Decision:** `DEFER_RUNTIME_TO_VS1_00`.
- **Target lot:** `DRIFT-IV-VS1-00`.
...
- **QA evidence:** none applicable — a document is reviewed, not QA-tested. Identity is approved and closed; only listening/QA follow-up and `DRIFT-IV-VS1-00`'s own owner acceptance remain open, per the corrected §21.
...
- **Target lot:** `DRIFT-IV-GOV-10` (this lot, document only). Runtime remains deferred to `DRIFT-IV-VS1-00`.
...
- **Decision:** **split by architectural role**, per the Integral Systems Architecture's own boundary between shared services and track-local scenes. `src/lib/drift3dAudioClock.ts` → `DEFER_RUNTIME_TO_SYS_00` (it is the shared audio-clock service, not EUX-specific). `src/lib/drift3dEuxGainentCues.ts`, `EuxGainentLivingScene.tsx`, and the `Drift3DCanvas.tsx` / `Drift3DClient.tsx` / `Drift3DScene.tsx` wiring → `DEFER_RUNTIME_TO_VS1_00`. `docs/ACTIVE_LOT.md` → `SUPERSEDED_BY_MAIN`; `docs/DECISIONS_LOG.md` entry → `HISTORICAL_EVIDENCE_ONLY`.
- **Target lot:** `DRIFT-IV-SYS-00` (audio clock) and `DRIFT-IV-VS1-00` (EUX cue wiring and scene).
...
| Fichier | État historique | État sur main | Valeur | Risque | Décision | Lot cible |
|---|---|---|---|---|---|---|
| `src/components/drift-3d/Drift3DWorldEdges.tsx` | Created by `92f4bfd`, reworked by `740e437` | Absent | High (superseded internally by its own rework) | Architectural divergence vs. `main`'s heightfield terrain (`drift3dTerrain.ts`) | `DEFER_RUNTIME_TO_BASE_00` | `DRIFT-IV-BASE-00` |
| `src/lib/drift3dRivers.ts` | Created by `740e437` | Absent | Medium | Same terrain-model divergence as above | `DEFER_RUNTIME_TO_BASE_00` | `DRIFT-IV-BASE-00` |
| `src/lib/drift3dScatter.ts` (river-corridor exclusion hunk) | Modified by `740e437` | Independently evolved (10 scatter archetypes) | Low-medium | Depends on `drift3dRivers.ts` above | `DEFER_RUNTIME_TO_BASE_00` | `DRIFT-IV-BASE-00` |
| `src/components/drift-3d/Drift3DScatterField.tsx` (wind hunk) | Modified by `d77edcf` | Independently evolved instanced scatter field | Medium-high | Shader-patch compatibility with `main`'s current material/instancing setup unverified | `DEFER_RUNTIME_TO_BASE_00` | `DRIFT-IV-BASE-00` |
| `src/lib/drift3dLandmarks.ts` (track-flavor props hunk) | Modified by `6c2998b` | Independently evolved (photo-PBR figurative scenes, EUX/ÉTÉÉAOOÉTÉ landmarks added directly on `main`) | Medium | Duplicate/conflicting entries likely on blind cherry-pick | `DEFER_RUNTIME_TO_BASE_00` | `DRIFT-IV-BASE-00` |
...
| `src/lib/drift3dEuxGainentCues.ts` | Created by `ad21600` | Absent | High — EUX-local cue resolver | Depends on `SYS-00`'s audio clock landing first | `DEFER_RUNTIME_TO_VS1_00` | `DRIFT-IV-VS1-00` |
| `src/components/drift-3d/EuxGainentLivingScene.tsx` | Created by `a15fba9`, extended by `ad21600` | Absent | High | Never owner-accepted even on its own branch (`PENDING_OWNER_REVIEW`) | `DEFER_RUNTIME_TO_VS1_00` | `DRIFT-IV-VS1-00` |
| `src/components/drift-3d/Drift3DCanvas.tsx` (EUX wiring hunks) | Modified by `a15fba9`, `ad21600` | Independently evolved | Medium | Wiring only meaningful once `EuxGainentLivingScene.tsx` lands | `DEFER_RUNTIME_TO_VS1_00` | `DRIFT-IV-VS1-00` |
| `src/components/drift-3d/Drift3DClient.tsx` (audio clock sync hunk) | Modified by `ad21600` | Independently evolved | Medium | Depends on `SYS-00`'s audio clock | `DEFER_RUNTIME_TO_VS1_00` | `DRIFT-IV-VS1-00` |
| `src/components/drift-3d/Drift3DScene.tsx` (EUX mount + WorldEdges mount hunks) | Modified by `92f4bfd`, `a15fba9`, `ad21600` | Independently evolved (terrain, vehicle physics, scatter) | Medium | Mixed: its WorldEdges-mount hunk belongs with `BASE-00`; its EUX-mount hunk belongs with `VS1-00` — must be split, not ported as one hunk | `DEFER_RUNTIME_TO_VS1_00` (EUX mount) / `DEFER_RUNTIME_TO_BASE_00` (WorldEdges mount) | `DRIFT-IV-VS1-00` and `DRIFT-IV-BASE-00` respectively |

No file in this group is modified, created or ported by `DRIFT-IV-GOV-10`.
...
|---|---|---|
| `DRIFT-IV-BASE-00` | `Drift3DWorldEdges.tsx`, `drift3dRivers.ts`, `drift3dScatter.ts` (river exclusion), `Drift3DScatterField.tsx` (wind), `drift3dLandmarks.ts` (track-flavor props), `Drift3DScene.tsx` (WorldEdges mount) | World-edge depth, vegetation wind and landmark detail candidates; require architecture reconciliation against `main`'s own heightfield terrain and scatter systems before any porting |
| `DRIFT-IV-SYS-00` | `drift3dAudioClock.ts` | Shared audio-clock service, track-agnostic per the Integral Systems Architecture |
| `DRIFT-IV-VS1-00` | `EuxGainentLivingScene.tsx`, `drift3dEuxGainentCues.ts`, `Drift3DCanvas.tsx` (EUX hunks), `Drift3DClient.tsx`, `Drift3DScene.tsx` (EUX mount hunk) | EUX GAINENT vertical-slice runtime — track-local scene, cue resolver and wiring, gated by the now-adopted Identity Contract and Cue Map |

## 4.1 Runtime status of EUX GAINENT

**Runtime status: NOT YET INTEGRATED ON MAIN.** The implementation candidate is preserved at `ad21600` (and the intermediate states at `a15fba9`, `46930a1`, `5eed84a`) and must be reimplemented or selectively ported in `DRIFT-IV-VS1-00`, against `main`'s current `Drift3DScene.tsx` / `Drift3DCanvas.tsx` / `Drift3DClient.tsx`, not cherry-picked as-is.
...
- the EUX Identity Contract no longer implies it assigns timing itself (`Timing must come from an owner-led listening pass` → it now explicitly defers all timing authority to `DRIFT_3D_EUX_GAINENT_CUE_MAP.md`, with bounded calibration adjustments possible before `DRIFT-IV-VS1-00` acceptance);
- the EUX Cue Map no longer frames itself as pending future approval (`Timing data ready for implementation after owner approval` → `Owner-approved initial timing data for implementation`; `No cue may become runtime authority before owner approval` → owner approval is already recorded for the initial baseline, and the cues become delivered runtime only after `DRIFT-IV-VS1-00` implementation, QA and acceptance).
...
- Deletion of any of these refs can only be decided after the runtime candidates deferred above have been recovered and accepted in `DRIFT-IV-SYS-00` and `DRIFT-IV-VS1-00`. No new tag was created by this lot; the existing anchor is sufficient.
