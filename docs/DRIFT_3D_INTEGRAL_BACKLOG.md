...
| `DRIFT-IV-PRE-20` | `READY_AFTER_MERGE` (blocked only on `DRIFT-IV-PRE-10`'s own merge; not started) |
...
**`BASE-00` à `SYS-70` sont tous `DONE`. SHARED PRE-GATE FOUNDATION COMPLETE.** `DRIFT-IV-BY-EUX-20`, `DRIFT-IV-BY-EUX-30`, `DRIFT-IV-GOV-40` **et `DRIFT-IV-PRE-00`** sont désormais tous `DONE` — voir §7.1 (`BY-EUX-20` mergé PR #31 à `d2a1c15` ; `BY-EUX-30` owner-accepted et mergé PR #32 à `b069d09` ; `GOV-40` mergé PR #33 à `99eacbe` ; `PRE-00` mergé PR #34 à `8a74a31`, corrigé `DRIFT-IV-PRE-10`, superseding l'état `DONE_PENDING_MERGE` précédemment enregistré ici). Le lot actif est désormais `DRIFT-IV-PRE-10` (§8.2) — production des cinq masterframes visuels réels — désormais `DONE_PENDING_MERGE` : décision finale du propriétaire, les cinq masterframes sont `ACCEPT` (Entry, Birth Yard, Older Shadows, Vegetative Field, New Signal), superseding les verdicts `REWORK`/`REJECT` du round 1. Les cinq fichiers finaux sont normalisés techniquement à exactement 1664×936 (16:9 exact), WebP/RGB, profil sRGB explicite, cinq hashes SHA-256 distincts. `DRIFT-IV-PRE-20` est `READY_AFTER_MERGE`, non démarré.
...
PRE-20 depends on: PRE-10 reaching DONE_PENDING_MERGE (satisfied — PRE-10 is DONE_PENDING_MERGE; PRE-20 itself is READY_AFTER_MERGE, blocked only on PRE-10's own merge, not started)
...
→ DRIFT-IV-PRE-10 — Five real visual masterframes, DONE_PENDING_MERGE (final owner decision: all five ACCEPT — see docs/evidence/DRIFT-IV-PRE-10/)
```
