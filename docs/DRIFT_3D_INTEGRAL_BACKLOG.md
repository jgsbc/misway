...
**`BASE-00` à `SYS-70` sont tous `DONE`. SHARED PRE-GATE FOUNDATION COMPLETE.** `DRIFT-IV-BY-EUX-20`, `DRIFT-IV-BY-EUX-30`, `DRIFT-IV-GOV-40` **et `DRIFT-IV-PRE-00`** sont désormais tous `DONE` — voir §7.1 (`BY-EUX-20` mergé PR #31 à `d2a1c15` ; `BY-EUX-30` owner-accepted et mergé PR #32 à `b069d09` ; `GOV-40` mergé PR #33 à `99eacbe` ; `PRE-00` mergé PR #34 à `8a74a31`, corrigé `DRIFT-IV-PRE-10`, superseding l'état `DONE_PENDING_MERGE` précédemment enregistré ici). `DRIFT-IV-PRE-10` a suivi, désormais `DONE — MERGED` (PR #35, `main@e212c05`) : décision finale du propriétaire, les cinq masterframes sont `ACCEPT` (Entry, Birth Yard, Older Shadows, Vegetative Field, New Signal), superseding les verdicts `REWORK`/`REJECT` du round 1, cinq fichiers finaux normalisés à exactement 1664×936 (16:9 exact), WebP/RGB, sRGB explicite, cinq hashes SHA-256 distincts. Le lot actif est désormais `DRIFT-IV-PRE-20` (§8.2) — registre d'actifs sous licence et évaluation d'import — désormais `DONE_PENDING_MERGE` : décision finale du propriétaire, ensemble promu de 5 candidats accepté avec rôles `PILOT`/`ADOPT` bornés par des garde-fous explicites (1 `ADOPT` Poly Haven `snow_02`, 4 `PILOT` couvrant les trois groupes de domaine `PRE-30`) ; les 2 `REJECT` motivés (Mixamo, Kenney Nature Kit) acceptés avec portée exacte ; la clôture rétroactive de licence Poly Haven acceptée comme gouvernance uniquement ; le compte-rendu Quaternius corrigé (seul le point de distribution Google Drive était bloqué, le site officiel restant pleinement accessible). `DRIFT-IV-PRE-30` est `READY_AFTER_MERGE`, non démarré.
...
|---|---|---|
| `PRE-00` | **Canonical artistic reconciliation and owner acceptance** — the Track Atlas (27 segments), the Global Art Direction doctrine (including the density, human-presence, transition-principles, gold/silver and lambda-progression additions), all five Masterframe Briefs (New Signal `ACCEPT WITH GUARDRAIL`, corrected to 75/75 explicit canonical fields after an owner-rejected waiver), the three owner-resolved artistic conflicts (Blossoming/Daymason/Hold The Light), Panthere's `P2`→`P3` reclassification, and this 153-lot resequencing itself — all owner-`ACCEPT`ed, final. **`DONE` — merged PR #34 at `8a74a31`.** | Documentation / owner review only |
| `PRE-10` | **Redefined:** production and owner acceptance of **five real visual masterframes** — actual rendered or concept-art reference images for Entry/Birth Yard/Older Shadows/Vegetative Field/New Signal. The text briefs in `docs/DRIFT_3D_MASTERFRAME_BRIEFS.md` are this lot's own input spec, not its deliverable. **`DONE_PENDING_MERGE`:** five candidates were produced externally, ingested, technically validated and visually audited; the owner reviewed the actual images directly and issued a final explicit decision — all five `ACCEPT` (Entry, Birth Yard, Older Shadows, Vegetative Field, New Signal) — superseding an intermediate round-1 pass that had returned four `REWORK`/`REJECT` verdicts. All five final files are technically normalized to exactly 1664×936 (exact 16:9), WebP/RGB, explicit sRGB ICC profile, five distinct SHA-256 hashes; see `docs/evidence/DRIFT-IV-PRE-10/` §16 for the full final record. | **Real visual-production lot** (a meaningful scope change from the prior text-only review) |
| `PRE-20` | Licensed asset/provenance registry and import evaluation: evaluates the `REFERENCE` candidates in `docs/DRIFT_3D_ASSET_REUSE_MATRIX.md`, obtains and records real licence/provenance evidence, promotes confirmed items to `ADOPT`/`PILOT`. No asset adopted without recorded licence/provenance (standing rule, `DRIFT_3D_INTEGRAL_SYSTEMS_ARCHITECTURE.md` §8.2). **`DONE_PENDING_MERGE`:** owner accepted 1 `ADOPT` (Poly Haven `snow_02`) + 4 `PILOT` (Kenney Mini Characters, Kenney City Kit Commercial, Kenney Car Kit, three.js `Water.js`/`Sky.js`), each with an explicit bounded guardrail; accepted 2 `REJECT` (Mixamo, Kenney Nature Kit) with precisely scoped reasoning; Quaternius `REFERENCE` (Google Drive distribution endpoint blocked on 2 tested packs, official website itself fully accessible — not a licence issue). Full record: `docs/evidence/DRIFT-IV-PRE-20/`. | **Real runtime/asset lot** |
...
PRE-40 depends on: PRE-30 reaching DONE_PENDING_MERGE (satisfied — merged PR #37). PRE-40 itself is IN_PROGRESS: one continuous five-macro-world greybox built at /drift-greybox-lab (exactly 4 real transitions, 239.3m total route, canonical vehicle only, PRE-30 mechanisms reused not duplicated, zero new tracked assets), tests/lint/build/scans all clean, a real ambient-audio-autoplay defect found and fixed across both this lab and PRE-30's own /drift-kit-lab — but live-render verification (actual driving, transitions, screenshots) is honestly recorded as KNOWN_ENVIRONMENT_LIMITATION, not claimed. Provisional readiness dossier: all 5 worlds GO_WITH_GAPS, global recommendation provisionally READY_WITH_NON_BLOCKING_GAPS, all owner verdicts PENDING — see docs/evidence/DRIFT-IV-PRE-40/.
VF-MORNE-00 (proof slice 2) depends on: PRE-40 reaching DONE_PENDING_MERGE (not yet satisfied — PRE-40 is IN_PROGRESS). VF-MORNE-00 remains BLOCKED_BY_DEPENDENCY, not started.
```
...
→ DRIFT-IV-PRE-00 — Canonical artistic reconciliation and owner acceptance, DONE — MERGED (PR #34, main@8a74a31)
→ DRIFT-IV-PRE-10 — Five real visual masterframes, DONE — MERGED (PR #35, main@e212c05, final owner decision: all five ACCEPT — see docs/evidence/DRIFT-IV-PRE-10/)
→ DRIFT-IV-PRE-20 — Licensed asset/provenance registry and import evaluation, DONE_PENDING_MERGE (owner accepted the 5-candidate promoted set with bounded guardrails — see docs/evidence/DRIFT-IV-PRE-20/)
```
