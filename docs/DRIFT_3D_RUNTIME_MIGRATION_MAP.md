...
**`DRIFT-IV-PRE-40` addendum (2026-08-03):** the same three pilot mechanisms are now exercised *in situ*, across a full five-macro-world route (`/drift-greybox-lab`, `docs/evidence/DRIFT-IV-PRE-40/`) rather than in three isolated single-scene pilots — but **this is still not a migration into production `/drift`**, for the same reason as `PRE-30`'s own addendum above: none of it replaces `FoolfouleCrowd`, the bare `Reflector` flag, or any `EUX GAINENT` one-off in the actual shipped `/drift` route, confirmed by this lot's own production-route scope scan (zero `src/app/drift/**` or `Drift3DScene.tsx`/`Drift3DClient.tsx`/`Drift3DCanvas.tsx` change). One additional real technique proven this pass: a disposable `Reflector` canal (Birth Yard) as a *second*, better-disposed water technique alongside `PRE-30`'s own `Water.js` pilot (`Water.js`'s upstream disposal limitation, documented in `PRE-30` §5, is unchanged and still applies to New Signal's own reuse of it here). §6's own standing rule remains unchanged.
