...
Those capabilities are the next governed evolution. `DRIFT-IV-BASE-00` through `DRIFT-IV-SYS-70`, `DRIFT-IV-BY-EUX-20`, `DRIFT-IV-BY-EUX-30`, **and `DRIFT-IV-GOV-40`** are all `DONE` (`GOV-40` merged to `main` at commit `99eacbe`, PR #33 — corrected, `DRIFT-IV-PRE-00`, superseding this section's prior "not yet merged" language). **The canonical prerequisite chain from here** (`DRIFT_3D_INTEGRAL_BACKLOG.md` §8.2, as resequenced and accepted):

```text
DRIFT-IV-GOV-40 (DONE, merged PR #33 at 99eacbe)
→ DRIFT-IV-PRE-00 (canonical artistic reconciliation and owner acceptance — active lot)
...
`DRIFT-IV-PRE-00` is the active lot (documentation/evidence only, see `docs/evidence/DRIFT-IV-PRE-00/`); `DRIFT-IV-PRE-10` onward remain not started. The backlog governs the detailed execution order. Their existence must not be claimed until implemented, validated and accepted lot by lot.
