...
test("VEH-B01 original Safari 110 asset stays valid and self-owned", () => {
  ...
  assert.match(DRIFT_3D_ORIGINAL_VEHICLE.path, /^\/models\/vehicle-hero\/.+\.glb$/);
});

test("VEH-B01 remains a credible metric expedition 4x4 and bounded realtime asset", () => {
  ...
  assert.ok(asset.triangleCount > 10_000 && asset.triangleCount <= 60_000);
  assert.ok(asset.sizeBytes < 2_000_000);
});
