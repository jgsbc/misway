  ...
  return (
    <main className="fixed inset-0 isolate overflow-hidden bg-[#171714] text-[#f3efe5]">
          ...
          <dl className="mt-3 grid grid-cols-3 gap-3 border-t border-white/10 pt-3 font-mono text-[9px] uppercase tracking-[0.12em] text-white/48">
            <div>
              ...
              <dd className="mt-1 text-white/85">
                {candidate.triangleCount.toLocaleString("en-US")}
              </dd>
            </div>
            <div>
              <dt>Licence</dt>
              <dd className="mt-1 text-white/85">{candidate.license}</dd>
            </div>
            <div>
              <dt>Runtime</dt>
              <dd className="mt-1 text-white/85">{candidate.runtimeFit}</dd>
            </div>
          </dl>
        </div>
      </section>
      ...
      <nav className="pointer-events-none absolute right-[calc(1rem+env(safe-area-inset-right))] top-[calc(1rem+env(safe-area-inset-top))] z-20 flex flex-col items-end gap-2 md:right-6 md:top-6">
        <Link
          href="/drift-kit-lab/vehicle/"
          className="pointer-events-auto rounded-full border border-white/20 bg-black/65 px-3 py-1.5 font-mono text-[9px] uppercase tracking-[0.18em] text-white/75 backdrop-blur-md transition hover:bg-black/85"
        >
          Procedural study
        </Link>
        <Link
      ...
      <div className="pointer-events-none absolute bottom-[calc(1rem+env(safe-area-inset-bottom))] left-1/2 z-20 flex w-[min(94vw,58rem)] -translate-x-1/2 flex-wrap justify-center gap-2 md:bottom-6">
        {DRIFT_3D_REAL_VEHICLE_CANDIDATES.map((item) => (
          <button
            ...
            className={`pointer-events-auto min-h-10 rounded-full border px-4 py-2 font-mono text-[9px] uppercase tracking-[0.16em] backdrop-blur-md transition ${
              activeId === item.id
                ? "border-[#d7bd7b] bg-[#d7bd7b] text-black"
                : "border-white/25 bg-black/70 text-white/78 hover:bg-black/90"
            }`}
          >
            {item.label}
          </button>
      ...
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="absolute bottom-[calc(4.7rem+env(safe-area-inset-bottom))] right-[calc(1rem+env(safe-area-inset-right))] z-20 rounded-full border border-white/15 bg-black/60 px-3 py-1.5 font-mono text-[8px] uppercase tracking-[0.16em] text-white/60 backdrop-blur-md transition hover:text-white md:bottom-20 md:right-6"
      >
        Source / licence ↗
      </a>
