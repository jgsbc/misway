...
/**
 * Hero Asset Pass 01: one real authored GLB, placed as mundane delivery
 * pressure outside EUX GAINENT. Loading is client-only and failure-safe: the
 * existing Hero Slice remains fully usable if the remote pinned asset cannot
 * be reached.
 */
export default function BirthYardHeroAssets() {
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        // Original textures include the source branding. They are explicitly
        // discarded; only geometry enters the MISWAY visual language.
        ...
        if (sourceLength > 0) {
          const scale = truck.targetLength / sourceLength;
      ...
      (error) => {
        if (process.env.NODE_ENV !== "production") {
          console.warn("Birth Yard delivery hero asset failed to load", error);
        }
      }
    );
  ...
  return (
    <group
      ref={rootRef}
      position={[truck.x, groundY + 0.025, truck.z]}
      rotation={[0, truck.rotationY, 0]}
      aria-hidden="true"
    />
  );
}
